      ...
      <div class="py-2 -my-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
        <div class="inline-block min-w-full overflow-hidden align-middle border-b border-gray-200 shadow sm:rounded-lg">
          ...
          <table class="min-w-full">
            ...
            <template v-for="chat in chat_result.data">
...
import {RiInformation2Line, RiCheckboxCircleLine, RiCloseCircleLine} from "@remixicon/vue";
...
const get_chats_status = async () => {
  try {
    const res = await api.get(`/admin/chat-status`)
...
const get_chats = async (page = 1) => {
  try {
    const res = await api.get(`/admin/chats/${chat_selected_state.value}?page=${page}`)
...
onMounted(async () => {
  ...
  if (configStore.channel) {
    configStore.channel.bind('new-chat-pending', handle_new_pending)
  }
...
onUnmounted(() => {
  ...
  if (configStore.channel) {
    configStore.channel.unbind('new-chat-pending', handle_new_pending)
  }
})
